...
const char ssid[] = "Familia Gil"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "ContraseniaTommY512*"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "204.236.253.135"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
const int MQTT_PORT = 8082;
const char MQTT_USER[] = USER;
//Contraseña de MQTT 
const char MQTT_PASS[] = "Blue12345678*"; // TODO Reemplace ContrasenaMQTT por la contraseña correpondiente al usuario especificado.

//Tópico al que se recibirán los datos
// El tópico de publicación debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/out
const char MQTT_TOPIC_PUB[] = "COLOMBIA/CUNDINAMARCA/SOACHA/" USER "/out"; //TODO Reemplace el valor por el tópico de publicación que le corresponde.
// El tópico de suscripción debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/in
const char MQTT_TOPIC_SUB[] = "COLOMBIA/CUNDINAMARCA/SOACHA/" USER "/in"; //TODO Reemplace el valor por el tópico de suscripción que le corresponde.
 ...
 */
void sendSensorData(float temperatura, float humedad, float intensidad_luminica) {
  String data = "{";
  data += "\"temperatura\": "+ String(temperatura, 1) +", ";
  data += "\"humedad\": "+ String(humedad, 1)+", ";
  data += "\"intensidad_luminica\": "+ String(intensidad_luminica,1);
  data += "}";
...
float readIntensidadLuminica() {
  // Se lee la intensidad luminica
 
  //Lee los datos del sensor
  float i = analogRead(ldr_pin);
...
void measure() {
  if ((millis() - measureTime) >= MEASURE_INTERVAL * 1000 ) {
    ...
    humi = readHumedad();
    int_lum = readIntensidadLuminica();

    // Se chequea si los valores son correctos
    if (checkMeasures(temp, humi, int_lum)) {
      // Se envían los datos
      sendSensorData(temp, humi, int_lum); 
    }
  }
}
